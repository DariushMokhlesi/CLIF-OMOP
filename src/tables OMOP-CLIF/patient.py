...
omop_parquet_dir = config["clif_parquet_dir"]
...
def remove_columns_patient():
    try:
        person_df = pd.read_parquet(file_path)
        person_df = person_df.drop(columns=['year_of_birth', 'month_of_birth', 'day_of_birth', 'location_id', 'provider_id', 'care_site_id', 'person_source_value', 'gender_source_concept_id', 'race_source_concept_id', 'ethnicity_source_concept_id'])
        print(person_df.head())
    except Exception as e:
        print(f"Error processing file: {e}")
